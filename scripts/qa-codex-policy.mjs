...
assert(!document.querySelector(".wb-codex-perm-menu"), "settings popover is closed until clicked");
click(trigger);
await new Promise((r) => setTimeout(r, 40));
assert(Boolean(document.querySelector(".wb-codex-perm-menu")), "clicking the trigger opens the settings popover");
const presets = [...document.querySelectorAll(".wb-codex-perm-menu .wb-segment")].map((b) => b.textContent);
assert(presets.includes("Read Only") && presets.includes("Auto") && presets.includes("Full Access"), "the three presets are shown");
assert(document.querySelector(".wb-codex-perm-menu")?.textContent.includes("Guardian"), "guardian toggle is present");
click([...document.querySelectorAll(".wb-codex-perm-menu .wb-segment")].find((b) => b.textContent === "Full Access"));
await new Promise((r) => setTimeout(r, 40));
