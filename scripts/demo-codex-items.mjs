/*
 * Live demo launcher for Codex transcript items (Item 3). Launches the real app
 * on the LEFT monitor with the fake codex app-server, creates a Codex member,
 * sends a turn whose fake response emits every ThreadItem type, captures a
 * screenshot, and LEAVES the app running. Not a test — a demo aid.
 */
...
const ws = path.join(os.tmpdir(), "agentparty-items-demo");
const userData = path.join(os.tmpdir(), "agentparty-items-demo-user-data");
const port = 48936;
...
async function main() {
  ...
  await post("/api/parties", { name: "transcript item 시연" });
  await post("/api/party/members", { name: "codey", requirement: "transcript item 시연", runtime: "codex", model: "gpt-5.4-mini", permissionMode: "default" });
  await post("/api/party/members/codey/open", {});
  await post("/api/party/members/codey/send", { content: "KIND=items 작업을 보여줘" });
  for (let i = 0; i < 30; i += 1) {
    ...
    if (s && Number(s.snapshot?.turnCount || 0) >= 1) { console.log("TURN_COMPLETE cards rendered"); break; }
    await delay(400);
  }
  try {
    const shot = path.join(os.tmpdir(), "items-demo.png");
