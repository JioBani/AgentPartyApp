...
const skills = D.skillCommands({ data: [{ cwd: "/w", skills: [
  ...
  { name: "browser:control-in-app-browser", path: "C:\\fake\\browser\\control-in-app-browser\\SKILL.md", enabled: true },
  { name: "computer-use:computer-use", path: "C:\\fake\\computer-use\\SKILL.md", enabled: true },
], errors: [] }] });
assert(skills.length === 4 && skills[0].source === "skill", "skills/list → skill commands");
...
assert(
  skills.some((skill) => skill.name === "browser:control-in-app-browser" && !skill.disabledReason)
    && skills.some((skill) => skill.name === "computer-use:computer-use" && !skill.disabledReason),
  "Codex browser and Computer Use skills remain available in discovery",
);
...
assert(plugins.map((p) => p.name).join(",") === "formatter,blocked,browser", "all installed plugins remain discoverable");
...
assert(plugins[2].name === "browser" && !plugins[2].disabledReason, "Codex browser plugin remains available in discovery");
