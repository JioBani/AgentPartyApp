...
3. **Toolbar** (40px — hidden when panel is *narrow*): member dot + name + status pill (`working` pulses). Right side, **density-dependent** (see Responsive): model pill (opens Runtime), effort pill, the **auto-compact pill** (see Auto-compact), a restart icon, and a stop/restart action (stop = danger when working). **Note: the permission control lives in the composer, not here** (moved out of the toolbar).
...
5. **Composer** (bottom):
   - *wide/mid*: bordered box, 2-row textarea (`{member}에게 메시지 보내기…`), action row — attach, @-mention, then a divider and the **permission pill** (shield + current mode `ask/auto/read`, opens Runtime) — plus a Send/Stop button (Stop = danger when that member is working).
...
- **wide ≥ 600px**: full toolbar (model + effort pills, auto-compact pill, restart icon, stop), 2-row composer (with the permission pill in its action row), tool results auto-expanded.
...
- Right pane: selected model detail — name + provider chip; 3 stat cards (Performance bars + tier, Cost per-1M in/out + `$` meter, Context window); **Effort** segmented control (options vary per model); **Extended thinking** toggle (only for thinking-capable models); **Debug logging** toggle; **Auto-compact** block (toggle + threshold slider — same control described in the Auto-compact section, edits staged with the rest and written on Apply).
...
## Auto-compact (per-member context compaction)
Every member has an **auto-compact** setting: when its context usage crosses a threshold (expressed as a **% of the model's context window**), the session is automatically compacted. Manual and automatic compaction coexist — they are not separate modes.

**Toolbar pill** (wide density): a single segmented pill sitting after the effort pill, `[ compact-icon │ NN% ]` — one rounded container, a 1px center divider, same border/tint color throughout:
- **Left half (icon)** → runs a **manual compact now** (icon swaps to a spinning refresh glyph for ~1.4s while compacting).
- **Right half (number)** → opens the **threshold-edit modal** for that member.
- When auto-compact is **on**, the pill uses the `--live` tint (`--live-dim` bg, `--live-bd` border) and shows the threshold `NN%`. When **off**, it is neutral (`--border`, `--text-3`) and reads `OFF` — the icon still runs a manual compact.

**Threshold-edit modal** (~430px, opened from the pill number): header (compact icon + "Auto-compact" + target member dot/name), a **toggle** ("임계치 초과 시 자동 압축"; off = manual-only), and — when on — a **threshold slider** (range 50–95%, step 5) with a live readout `NN% · ≈ <tokens>K 토큰` (token estimate = window × %). Edits apply live to that member; "완료" closes. **Per-member thresholds are independent** (prototype seeds backend 80 / frontend 85 / reviewer off / tester 75 / db-migrate off / docs off).

**Visibility elsewhere**: members with auto-compact on show a small `⇲ NN%` mono badge in the **sidebar member row**, so the state is visible without opening the panel.

**Global default**: new members inherit a default (`{ on, at }`) set on the **Settings → Runtime** screen ("Auto-compact" section: toggle + default threshold slider). The workbench falls back to this `compactDefault` for any member without an explicit setting.
...
- `sidebarOpen: boolean`.
- `runtime: { open, targetMemberId, selModel, effort, thinking, debug, compactOn, compactAt, staged }`; `modelOverride: { memberId → modelName }`.
- `compact: { memberId → { on: boolean, at: number } }` (per-member auto-compact); `compactDefault: { on, at }` (inherited by new members, edited in Settings); `compacting: { memberId → boolean }` (transient manual-compact-in-progress flag); `compactEdit: { open, targetMemberId }` (threshold-edit modal).
...
Triggers: select tab → set panel.active + focus; member click → open/focus; resize handle → adjust adjacent weights; split → insert panel; model pill → open runtime seeded from member's current model; Apply → write modelOverride + compact + close; compact pill icon → manual compact (transient spinner); compact pill number → open threshold-edit modal (writes `compact[member]` live).
...
- `screenshots/` — 01 split (2 panels) · 02 three-zone · 03 responsive breakpoints (wide/mid/narrow) · 04 tab-drag state · 05 runtime modal · 06 auto-compact (toolbar pill + threshold-edit modal; note the permission control now in the composer).
