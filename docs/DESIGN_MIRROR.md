...
목업 자체에서 발견한 결함 하나(**정본에서 수정됨**): `workbench.html` · `runtime.html` ·
`settings.html` 의 `<div class="mock-overlay">` 들이 닫히지 않아 대화상자가 서로
중첩됐다. `#overlay-new-group` 을 열면 조상인 `#overlay-new-party` 가 숨겨진 상태라
아무것도 보이지 않았다. 지금은 각 오버레이 끝에 `</div>` 가 채워져 전부 형제 노드다.

내려받기 검증은 **정본 파일을 손대지 않은 상태로** 다시 돌려 확인한다(위 실측 방식).
목업을 고쳐 가며 맞추면 "맞았다"가 내 수정본에 대한 것이 되어 버린다.
