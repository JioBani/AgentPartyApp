...
| `POST /api/qa/update` `{state, latestVersion, releaseNotes, progress, releases:[…]}` | pin an **app-update** status and/or release history — the update pill, its dialog, the rail's 버전 dot and the 버전 screen, driven without publishing a release (a dev run cannot self-update at all). `{"reset":true}` restores the real updater. See `GET /api/update` in API.md for the shapes |
