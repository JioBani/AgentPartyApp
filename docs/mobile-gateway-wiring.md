...
`getStatus()`는 호출할 때마다 새로 만든다 — `emit` 직후에 폴링해도 늘어난 `events.seq`가 보인다.
(`emit`은 `broadcastToWorkspace` 위에 있어 매 이벤트마다 상태를 푸시하지 않는다. 그래서 푸시 채널인
`status# MobileGateway 접합 가이드
...
는 상태 전이에만 반응하고, 이벤트 카운터는 `getStatus()`로 읽어야 한다.)
HTTP 핸들러에서 await 없이 즉답할 수 있다.
...
| `--pair-ttl <분>` | **개발 전용.** QR 유효시간을 늘린다(기본 2분, 상한 10분 — 시그널링 서버의 페어링 창과 같다). QR은 페어링 권한 그 자체라 유출된 창이 그만큼 길어진다(02 §T3). 켜면 `pairing_ttl_extended` 보안 경고가 뜬다 |
