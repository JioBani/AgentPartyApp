...
- 다시 키거나 메뉴를 돌아다니면, 멤버의 대화가 닫혀있는 문제
  ...
  - 이때 대화는 남아있음
  - 그러면서 열려있는것으로 다시 인식됨
- 특정 권한으로 초기 세팅해서 멤버 생성 할 수 있는 기능 : 이거는 ai 에이전트가 멤버생성할 때도 도출되어야 함 
- 런타임의 기본값으로 권한 설정도 세팅 할 수 있는 기능
- 멤버가 다른 멤버의 권한 설정을 수정 할 수 있는 기능



- 기계적으로 턴을 꺠우는 기능
  - mcp 로 도구 노출 + 유저도 컨트롤 할 수 있도록
  - claude code 의 해당 도구가 정상적으로 동작하지 않아서 필요
- 한도가 차면 한도가 다시 돌아온 이후에 턴을 깨우는 기능
