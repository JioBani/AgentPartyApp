...
> 이미 검증이 끝난 master를 그냥 내보내는 것뿐이라면
> [빠른 공개 릴리스](./RELEASE_FAST.md) 한 줄이면 된다. 아래는 정본 설명이다.

### SemVer 자리 이름

AgentParty 버전은 `MAJOR.MINOR.PATCH` 형식을 쓴다. 예를 들어 `1.4.2`는
major 1, minor 4, patch 2다.

| 자리 | 이름 | 올리는 경우 | 예시 |
| --- | --- | --- | --- |
| 첫 번째 | **Major(메이저)** | 기존 API·저장 데이터·사용자 동작의 호환성을 깨는 변경 | `1.4.2` → `2.0.0` |
| 두 번째 | **Minor(마이너)** | 기존 호환성을 유지하는 새 기능 | `1.4.2` → `1.5.0` |
| 세 번째 | **Patch(패치)** | 기존 호환성을 유지하는 버그 수정·성능 개선·내부 정리 | `1.4.2` → `1.4.3` |

`0.x.y`는 1.0 이전 개발 단계다. 이 단계에서도 변경 범위는 같은 기준으로 분류하고,
...
4. 버전 커밋과 `v<version>` 태그를 원격 `master`에 fast-forward로 올린 뒤, 검증한
   ...
   준비·게시 스크립트가 성공한 뒤에는 생성 파일을 다시 열거나 해시·크기·버전·공개
