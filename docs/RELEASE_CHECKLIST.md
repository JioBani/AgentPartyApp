...
### 주요 변경 화면 캡처와 릴리스 본문 미리보기

사용자에게 중요한 UI 변경은 실제 배포본에서 대표 화면을 캡처한다. 재현 가능한 QA
데이터를 사용하고 개인 정보·토큰·실제 작업 내용이 보이지 않는지 확인한다. 새 기능만
보지 말고 기존 입력창, 메뉴, 권한 팝업 등과 겹치거나 조작을 가로채지 않는지도
검증한다. 캡처가 네이티브 브라우저 뷰를 포함하지 못하는 경우에는 빈 화면을 완성된
브라우저 화면인 것처럼 게시하지 말고 해당 뷰를 별도로 캡처하거나 한계를 명시한다.

선별한 이미지는 공개 `AgentParty-releases` 저장소의 `media/releases/v<version>/`에
올리고, 한국어 릴리스 본문에서 이미지마다 사용자 관점의 제목과 변경 설명을 붙여
표시한다. 설치 파일이나 업데이트 피드 네 자산에 이미지를 섞지 않는다. 게시 전에
이미지와 릴리스 본문 전체를 사용자에게 미리 보여주고, 공개 URL 접근과 본문 내
이미지 링크를 확인한다. 시각적 변경이 없는 릴리스에는 억지 이미지를 만들지 않는다.
