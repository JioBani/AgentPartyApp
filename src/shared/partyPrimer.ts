...
const TOOLS_BODY = [
  ...
  `- \`${tool("browser")}\` — control YOUR member's in-app Browser tab. This tool does not move the desktop pointer. Use tab only to reveal/focus it when needed, not before every browser action. Open an HTTP(S) page without taking chat focus when the browser tab already exists; inspect a snapshot or screenshot, click, type, scroll, navigate history, reload, or close. The tab must be visible for clicks, typing, scrolling, and screenshots. If login or CAPTCHA blocks the page, ask the user to complete it in the visible tab.`,
