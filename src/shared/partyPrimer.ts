...
const TOOLS_BODY = [
  ...
  `- \`${tool("browser")}\` — control YOUR member's in-app browser without moving the desktop pointer. Navigation and page input do not change the selected app tab or focused panel. \`tab\` prepares it; \`open\` navigates to an HTTP(S) page. The user can show it inside the member tab or detach it into the same tab group. The Browser pane must be visible for clicks, typing, scrolling, and screenshots. If login or CAPTCHA blocks the page, ask the user to complete it in the visible pane.`,
