...
const TOOLS_BODY = [
  ...
  `- \`${tool("send")}\` — send the same message to one or more party members; \`to\` accepts one name or an array. Your \`from\` is set automatically to \`{{member}}\` — never supply it. Omit both delivery flags to use your saved member override and then the Runtime default. Pass \`interrupt: true\` to cut in or \`queue: true\` only when you explicitly need to be QUEUED behind the recipient's current turn (a Codex member receives it at its next tool call). Never pass \`interrupt: false\`; some harnesses auto-fill it and AgentParty treats it as inherit so it cannot disable the saved setting.`,
  `- \`${tool("broadcast")}\` — send one message to every other member at once; pass \`exclude\` with member names to omit (same QUEUE-then-current-turn timing and delivery overrides as \`send\`).`,
  ...
  `- \`${tool("browser")}\` — control YOUR member's in-app Browser tab, not the desktop mouse. Use tab only to reveal/focus it when needed, not before every browser action. Open an HTTP(S) page without taking chat focus when the browser tab already exists; inspect a snapshot or screenshot, click, type, scroll, navigate history, reload, or close. The tab must be visible for clicks, typing, scrolling, and screenshots. Do not substitute desktop Computer Use or another browser backend. If login or CAPTCHA blocks the page, ask the user to complete it in the visible tab.`,
...
const DISCIPLINE_BODY = [
  ...
  "- **Keep tool output proportional to the decision.** For MCPs and other tools that may return large catalogs, lists, logs, transcripts, or repeated detail, request the smallest useful scope with filters, an exact member or item, summaries, pagination, or limits. Do not fetch full detail speculatively; widen only when the smaller result cannot resolve the decision.",
