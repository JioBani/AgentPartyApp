...
const partyDynamicToolDescriptions: Record<PartyToolName, string> = {
  ...
  browser: "Control YOUR in-app browser without moving the OS pointer. Navigation and page input do not switch the selected app tab or focused panel. tab prepares the browser; open navigates to an HTTP(S) URL. The user can view it inside the member tab or detach it into the same tab group; detach and merge explicitly change that layout. Inspect a text/controls snapshot or screenshot, click viewport coordinates, type into the focused field, scroll, navigate history, reload, or close. The Browser pane must be visible for clicks, typing, scrolling, and screenshots. Coordinates are relative to its viewport. Login and CAPTCHA are for the user to complete; never claim a blocked page succeeded.",
