...
const partyDynamicToolDescriptions: Record<PartyToolName, string> = {
  ...
  browser: "Control YOUR in-app browser tab only; this tool does not move the OS pointer. Use tab only when you need to reveal/focus the browser, not before every action; open navigates to an HTTP(S) URL without stealing chat focus if the tab already exists. Merge returns a separated browser tab to your chat panel. Inspect a text/controls snapshot or screenshot, click viewport coordinates, type into the focused field, scroll, navigate history, reload, or close. The Browser tab must be visible for clicks, typing, scrolling, and screenshots. Coordinates are relative to its viewport. Login and CAPTCHA are for the user to complete; never claim a blocked page succeeded.",
