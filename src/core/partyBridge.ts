...
const partyDynamicToolSchemas: Record<PartyToolName, Record<string, unknown>> = {
  ...
  browser: {
    type: "object",
    properties: {
      action: { type: "string", enum: ["state", "open", "back", "forward", "reload", "snapshot", "screenshot", "click", "type", "scroll", "close"] },
...
export async function invokePartyTool(bridge: PartyBridge, identity: PartyIdentity, tool: string, args: unknown): Promise<PartyToolResult> {
  ...
  switch (name) {
    ...
    case "browser": {
      ...
      if (typeof action !== "string" || !["state", "open", "back", "forward", "reload", "snapshot", "screenshot", "click", "type", "scroll", "close"].includes(action)) {
...
export function buildPartyToolDefs(tool: ToolFactory, bridge: PartyBridge, identity: PartyIdentity): unknown[] {
  ...
  return [
    ...
    tool(
      ...
      {
        action: z.enum(["state", "open", "back", "forward", "reload", "snapshot", "screenshot", "click", "type", "scroll", "close"]),
